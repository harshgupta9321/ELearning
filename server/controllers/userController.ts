...
export const loginUser = catchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!email || !password) {
            return next(new ErrorHandler("Please enter you login credentials", 400))
        }
...
export const updateAccessToken = catchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const message = "Could not update refresh token"
        if (!decoded) {
        ...
        await redis.set(user._id,JSON.stringify(user),"EX",604800)     //604800 means 7days
...
//update access_token on reload I make it myself
export const updateAccessTokenOnReload = catchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        const refresh_token = req.cookies.refresh_token as string;
        const decoded = jwt.verify(refresh_token, process.env.REFRESH_TOKEN as string) as JwtPayload
...
export const updateProfilePicture = catchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if(avatar && user){
            //if user have avatar already then we have to remove it first before uploading
